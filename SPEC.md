...
Three batches, so no T39a is needed. The first batch is the largest on purpose: it is the one that settles whether the new topics survive contact with a solver, `geometry.grid` above all. It is written in four parts, T37.1 to T37.4, each reviewed on its own; `geometry.grid` goes first. The second is written in three: T38.1 for `logic.ordering` and `logic.knights_liars`, T38.2 for `combinatorics.enumeration` and `counting.gaps`, T38.3 for `time.clocks`.
